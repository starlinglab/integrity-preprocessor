...
while 1:
    ...
    for archive in response.json()["archives"]:
        ...
        response = requests.get("http://127.0.0.1:9871/api/archives/" + aid + "/crawls", headers=headers)
        for crawl in response.json()['crawls']:
            ...
            if crawl["state"] == "complete":
                ...
                if finishDate.timestamp() > lastCheck[aid]:
                    ...
                    response = requests.get("http://127.0.0.1:9871/api/archives/" + aid + "/crawls/" + crawl["id"] + ".json", headers=headers)
                    crawlReponse = response.json()

                    #path = SOURCEPATH + "/" + BUCKET + "/" + aid + '/data/' + startDateString + "*" + crawl["id"] + '.wacz'
                    waczPath = SOURCEPATH + "/" + BUCKET + "/" + crawlReponse["resources"][0]["name"]
                    meta = {}
                    with ZipFile(waczPath, 'r') as waczFile:
                        with tempfile.TemporaryDirectory() as td:
                            ...
                            with open(td + '/datapackage-digest.json') as json_file:
                                ...
                                print(data)
                                meta['authsign_software'] = data['signedData']['software']
        ...
        #                        print(data)
                            ...
                            zipObj = ZipFile(td + '/' + crawlReponse['id'] + '.zip', 'w')
                            zipObj.write (waczPath, basename(waczPath))
                            zipObj.write (td + "/metadata.json", "metadata.json")
                            zipObj.write (td + "/metadata_injestor.json", "/metadata_injestor.json")
                            zipObj.close

                            shutil.move(td + '/' + crawlReponse['id'] + '.zip', TARGETPATH + '/' + crawlReponse['id'] + '.zip.part')
                            os.rename(TARGETPATH + '/' + crawlReponse['id'] + '.zip.part', TARGETPATH + '/' + crawlReponse['id'] + '.zip')
