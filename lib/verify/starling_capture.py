...
class StarlingCapture:
    ...
    def verify(self, asset_path: str, meta_raw: str, sigs: dict) -> bool:
        """Verify hashes and signatures for Starling Capture data.
        ...
        return self._verify_create_hashes(
            asset_path, meta, sigs
        ) and self._verify_all_sigs(meta_raw, sigs)

    def _verify_create_hashes(self, asset_path: str, meta: dict, sigs: dict) -> bool:
    ...
    def _verify_all_sigs(self, meta_raw: str, signatures: dict) -> bool:
        ...
        for sig in signatures:
            if sig["provider"] == "AndroidOpenSSL":
                if not self._verify_androidopenssl(meta_raw, sig):
                    return False
            elif sig["provider"] == "Zion":
                if not self._verify_zion(meta_raw, sig):
                    return False
            else:
    ...
    def _verify_androidopenssl(self, meta_raw: str, signature: dict) -> bool:
    ...
    def _verify_zion(self, meta_raw: str, signature: dict) -> bool:
